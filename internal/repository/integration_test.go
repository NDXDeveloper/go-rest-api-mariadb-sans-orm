...
//
// Ces tests dialoguent avec une VRAIE base MariaDB de test (le conteneur
// « mariadb-test » exposé sur 127.0.0.1:13306, avec le jeu de données de
// démonstration). Ils ne sont compilés et exécutés QUE lorsque l'étiquette de
...
// les tests d'intégration flexibles : par défaut ils visent le conteneur local
// « mariadb-test » (port 13306), mais on peut les rediriger vers n'importe quelle
// base (ex. celle de « docker compose » sur le port 3306, ou le service MariaDB
// de la CI) en définissant BDD_HOTE, BDD_PORT, BDD_MOT_DE_PASSE, etc.
...
func connecterTest(t *testing.T) *sql.DB {
	t.Helper()
	port, err := strconv.Atoi(envTest("BDD_PORT", "13306"))
	if err != nil {
	...
	if err != nil {
		t.Skipf("MariaDB de test indisponible : %v", err)
	}
...
func TestIntegrationEmprunterEtRendre(t *testing.T) {
	...
	t.Cleanup(func() {
		if _, err := db.ExecContext(context.Background(), "DELETE FROM emprunts WHERE uuid = ?", empruntUUID); err != nil {
