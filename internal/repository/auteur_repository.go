...
// Creer insère un auteur (UUID généré par la couche service).
func (r *AuteurRepository) Creer(ctx context.Context, a *models.Auteur) error {
	...
	resultat, err := r.db.ExecContext(ctx, requete,
		a.UUID, a.Nom, a.Prenom, a.Nationalite, argDate(a.DateNaissance), a.Biographie)
	if err != nil {
		return apperreur.Interne("création de l'auteur").AvecCause(err)
	}
	id, err := resultat.LastInsertId()
	if err != nil {
		return apperreur.Interne("récupération de l'identifiant auteur").AvecCause(err)
	}
	a.ID = id
	return nil
}
...
func (r *AuteurRepository) ParUUID(ctx context.Context, uuid string) (*models.Auteur, error) {
	const requete = `SELECT ` + colonnesAuteur + ` FROM auteurs WHERE uuid = ?`
...
func (r *AuteurRepository) Lister(ctx context.Context, params models.ParametresListe) ([]models.Auteur, int, error) {
	...
	if params.Recherche != "" {
		...
		conditions.ajouter("(nom LIKE ? OR prenom LIKE ?)", motif, motif)
	}
...
func (r *AuteurRepository) MettreAJour(ctx context.Context, a *models.Auteur) error {
	const requete = `UPDATE auteurs SET nom = ?, prenom = ?, nationalite = ?, date_naissance = ?, biographie = ?
		WHERE uuid = ?`
	resultat, err := r.db.ExecContext(ctx, requete,
...
func (r *AuteurRepository) Supprimer(ctx context.Context, uuid string) error {
	const requete = `DELETE FROM auteurs WHERE uuid = ?`
