...
func (r *CategorieRepository) Creer(ctx context.Context, c *models.Categorie) error {
	const requete = `INSERT INTO categories (uuid, nom, description) VALUES (?, ?, ?)`
	resultat, err := r.db.ExecContext(ctx, requete, c.UUID, c.Nom, c.Description)
	if err != nil {
	...
	if err != nil {
		return apperreur.Interne("récupération de l'identifiant catégorie").AvecCause(err)
	}
	c.ID = id
	return nil
}
...
func (r *CategorieRepository) ParUUID(ctx context.Context, uuid string) (*models.Categorie, error) {
	const requete = `SELECT ` + colonnesCategorie + ` FROM categories WHERE uuid = ?`
...
func (r *CategorieRepository) Lister(ctx context.Context, params models.ParametresListe) ([]models.Categorie, int, error) {
	...
	if params.Recherche != "" {
		conditions.ajouter("nom LIKE ?", "%"+params.Recherche+"%")
	}
...
func (r *CategorieRepository) MettreAJour(ctx context.Context, c *models.Categorie) error {
	const requete = `UPDATE categories SET nom = ?, description = ? WHERE uuid = ?`
...
// Supprimer efface une catégorie (bloqué par la FK si des livres l'utilisent).
func (r *CategorieRepository) Supprimer(ctx context.Context, uuid string) error {
	const requete = `DELETE FROM categories WHERE uuid = ?`
