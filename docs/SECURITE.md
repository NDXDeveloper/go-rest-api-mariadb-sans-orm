...
Chaque valeur passe par un **paramètre `?`**, envoyé **séparément** de la requête au serveur, qui
la traite comme une **donnée** et jamais comme du code. On ne concatène **jamais** une valeur
utilisateur dans une requête (voir l'en-tête de `internal/repository/commun.go`).

```go
// internal/repository/utilisateur_repository.go
const requete = `SELECT ... FROM utilisateurs WHERE email = ? AND supprime_le IS NULL`
r.db.QueryRowContext(ctx, requete, email)   // « email » ne peut pas altérer la requête
```

Même la recherche `LIKE` reste paramétrée :

```go
// internal/repository/livre_repository.go
conditions.ajouter("titre LIKE ?", "%"+params.Recherche+"%")   // le motif est un ARGUMENT
```

**Le cas particulier du `ORDER BY` : la liste blanche.** Une valeur peut être un `?`, mais **pas**
un nom de colonne ni le sens du tri (`ASC`/`DESC`) : ceux-ci font partie de la **structure** de la
requête. Interpoler directement un nom de colonne fourni par le client rouvrirait la faille. La
parade est une **liste blanche** : le client envoie un nom logique (`titre`), traduit en nom de
colonne réel **uniquement** s'il figure dans la liste autorisée.
...
// internal/handler/commun.go — seule une colonne validée est retenue
...
Ainsi, `ColonneTri` ne contient **jamais** une valeur brute : toujours une colonne validée. Le sens
du tri est borné à `ASC`/`DESC` (voir `models.ParametresListe.Normaliser`).

**Défenses en profondeur supplémentaires :**

- `interpolateParams=false` (valeur par défaut du pilote, laissée telle quelle dans
  `internal/database/database.go`) : les paramètres sont envoyés séparément (requêtes préparées
  **côté serveur**), ce qui neutralise l'injection au niveau du protocole.
- `multiStatements` **non activé** : impossible d'empiler plusieurs requêtes (`; DROP TABLE …`) dans
  un même appel.
- **Moindre privilège** en base : l'utilisateur `app_bibliotheque` n'a ni `DROP`, ni `ALTER`, ni
  `CREATE`, ni `GRANT` (voir `sql/schema/01_privileges.sql`). Même une injection réussie ne pourrait
  pas détruire le schéma.
...
- **Les mots de passe ne sortent jamais du serveur.** Le champ `MotDePasseHash` porte `json:"-"`
  ...
  photos JSON (voir `sql/triggers/07_triggers.sql`).
...
- Refresh tokens **hachés** en base (SHA-256), **rotation** à chaque rafraîchissement (l'ancien est
  révoqué), purge automatique des expirés/révoqués par un **event** MariaDB horaire.
...
**Contraintes en base (défense en profondeur).** `CHECK`, `UNIQUE`, `ENUM`, clés étrangères et
triggers `SIGNAL` garantissent l'intégrité **même si** un bug applicatif passait au travers (voir
[DATABASE.md](../DATABASE.md)).

**Moindre privilège SQL.** L'utilisateur applicatif n'a que le CRUD + `EXECUTE` + `SHOW VIEW`,
jamais `DROP`/`ALTER`/`CREATE`/`GRANT` (voir `sql/schema/01_privileges.sql`).
...
|------------------------|------------------------------------------------------|-----------------------------------------------|
| Injection SQL          | Requêtes préparées `?` + liste blanche `ORDER BY`    | `repository/*`, `handler/commun.go`, `models/liste.go` |
...
| Intégrité              | `CHECK`/`UNIQUE`/FK/`SIGNAL` + moindre privilège SQL  | `sql/schema/*`, `sql/triggers/*`              |
