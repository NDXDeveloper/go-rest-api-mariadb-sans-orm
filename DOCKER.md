...
|-------------------------|------------------------------|-------------------------------------------------------------|
| Service `mariadb`       | conteneur `bibliotheque_mariadb` | Base MariaDB 11, initialisée par les scripts `sql/`     |
| Service `api`           | conteneur `bibliotheque_api`     | L'API Go, démarrée **après** que MariaDB soit « healthy » |
| Réseau (bridge)         | `<projet>_reseau_bibliotheque`   | Isole la communication API ↔ base                       |
| Volume nommé            | `<projet>_donnees_mariadb`       | **Persiste** les données MariaDB (`/var/lib/mysql`)     |

Le préfixe `<projet>` vient de `COMPOSE_PROJECT_NAME` (dans `.env`) ou, à défaut, du nom du dossier.

Points de conception importants :

- **`depends_on … condition: service_healthy`** : l'API ne démarre qu'une fois la base saine, ce
  qui évite les erreurs de connexion au tout premier lancement.
- **Scripts d'init montés dans `/docker-entrypoint-initdb.d/`** : au **premier** démarrage (volume
  vide), l'image MariaDB exécute `01` → `09` **dans l'ordre**. Aux démarrages suivants (volume déjà
  peuplé), ils **ne** sont **pas** rejoués.
- **`--event-scheduler=ON`** : active l'ordonnanceur d'events dès le démarrage du serveur.

---

## 1. Construire les images

### Construire l'image de l'API (via Compose)

```bash
docker compose build            # construit le service « api » (utilise le cache)
docker compose build --no-cache api   # reconstruction totale, sans cache de couches
```

- `build` construit l'image définie par le `Dockerfile` (build **multi-stage** : une étape
  compile le binaire Go, une étape produit une image finale minimale ~20 Mo, non-`root`).
...
```bash
docker build -t bibliotheque-api:local .
```
...
> **Cache et rapidité.** Le `Dockerfile` copie d'abord `go.mod`/`go.sum` et télécharge les  
...
- `--build` : (re)construit l'image de l'API avant de démarrer.
...
- **`down`** : supprime conteneurs et réseau mais **garde le volume nommé** : vos données MariaDB
...
docker compose logs --tail=100 mariadb   # les 100 dernières lignes de MariaDB
...
```bash
# Client interactif MariaDB (mot de passe root pris dans l'environnement du conteneur)
docker compose exec mariadb mariadb -u root -p"$BDD_ROOT_MOT_DE_PASSE" bibliotheque
```

### Vérifier events, procédures, triggers

```bash
docker compose exec mariadb mariadb -u root -p"$BDD_ROOT_MOT_DE_PASSE" bibliotheque \
  -e "SHOW EVENTS; SHOW TRIGGERS; SHOW PROCEDURE STATUS WHERE Db='bibliotheque';"
```

### Ouvrir un shell dans un conteneur

```bash
docker compose exec mariadb bash     # MariaDB (image Debian : bash disponible)
...
> run --rm mariadb <cmd>` démarre un conteneur **jetable** à partir de l'image du service  
...
### Sauvegarde / restauration (rappel)
...
docker volume ls                  # liste des volumes (repérez <projet>_donnees_mariadb)
...
Les données MariaDB vivent dans un **volume nommé** qui **survit** à `docker compose down`. Pour
...
docker volume rm bibliotheque_donnees_mariadb
```
...
> **sauvegarde** (`./scripts/backup.sh`) au préalable si nécessaire.
...
```bash
# Via Compose : supprime les images CONSTRUITES localement par ce projet (l'API)
docker compose down --rmi local

# Supprime aussi les images tirées (« pulled »), comme mariadb:11
docker compose down --rmi all

# Manuellement, par nom/identifiant
docker image rm bibliotheque-api:local
docker image rm mariadb:11
```

- `--rmi local` : ne retire que les images **buildées** par le projet (garde `mariadb:11` en cache).
- `--rmi all` : retire aussi les images de base téléchargées.
...
> Après une réinitialisation du volume, les scripts `sql/01…09` sont **rejoués** : vous retrouvez  
> le schéma complet et le jeu de données de démonstration.
...
| Exécuter une commande            | `docker compose exec mariadb …`    | `docker-compose exec mariadb …`    |
...
| …ouvrir un client SQL                                       | `docker compose exec mariadb mariadb -u root -p"…" bibliotheque` |
