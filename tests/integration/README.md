...
| **Intégration — repositories** | `internal/repository/integration_test.go` | **MariaDB réelle** |
...
# Tests d'INTÉGRATION (nécessitent une base MariaDB accessible)
...
Les tests d'intégration se connectent à une base MariaDB **réelle** contenant le
jeu de données de démonstration. Par défaut ils visent `127.0.0.1:13306`, mais
tout est configurable par variables d'environnement :

```bash
# Exemple : cibler la base lancée par « docker compose up » (exposée sur 3306)
BDD_HOTE=127.0.0.1 BDD_PORT=3306 \
...
> Astuce : pour une base de test jetable, lancez un conteneur MariaDB dédié sur  
> le port 13306 avec les scripts d'initialisation de `sql/`, ou réutilisez la  
> pile Docker Compose du projet.
