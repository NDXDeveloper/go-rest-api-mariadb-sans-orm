...
- Un client MariaDB/MySQL (optionnel), pratique pour inspecter la base pendant
  le développement.
...
cd go-rest-api-mariadb-sans-orm

# 2. Copiez le modèle de configuration et adaptez les secrets locaux
cp .env.example .env

# 3. Démarrez la pile complète (API + MariaDB), schéma et données de départ
...
Docker) si vous disposez d'une instance MariaDB accessible : dans ce cas,
...
base MariaDB disponible. Démarrez d'abord la base (par exemple via
`make demarrer`, ou `docker compose up -d mariadb`), puis lancez :
...
à votre instance MariaDB locale (voir `.env.example`).
